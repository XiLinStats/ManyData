...
gsar1 <- function(theta_e, theta_o, Sigma_e=diag(p), a=p - 2) {
  ...
  if (!any(is.na(Sigma_e)) && rcond(Sigma_e) > 1e-16) {
    iSigma <- solve.default(Sigma_e)
  }
  else {
    iSigma <- tryCatch(MASS::ginv(Sigma_e), error = function(e) {cat("ERROR : Sigma_e is singular", "\n")})
  }
...
gsar2 <- function(theta_e, theta_o, Sigma_e=diag(p), a=p - 2) {
  ...
  scl <- pmax(0, (diag(p) - a*iSigma/c( t(theta_e - theta_o) %*% iSigma2 %*% (theta_e - theta_o) )))
  dim(scl) <- c(p,p)
...
##' @export
rbob_kappa2 <- function(theta_e, theta_o, Sigma_e=diag(p)) {
  ...
  # kappa_2+
  lambda_2 <- sum(Sigma_e %*% Sigma_e) /
    (t(theta_e - theta_o) %*% Sigma_e %*% Sigma_e %*% (theta_e - theta_o))

  kappa2_plus <- c(theta_o +
                     pmax(diag(p) - as.numeric(lambda_2) * Sigma_e, 0) %*%
                     (theta_e - theta_o))
