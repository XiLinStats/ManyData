...
summ<-results[,.(.N,beta_1 = mean(beta_1),
              ...
              ATE_sd = sd(ATE),
              elpd = mean(ELPD_waic),
              RMSE_beta1 = sqrt((mean(beta_1) - beta_1_true)^2 + sd(beta_1)^2),
              RMSE_beta3 = sqrt((mean(beta_3) - beta_3_true)^2 + sd(beta_3)^2),
              RMSE_ATE = sqrt((mean(ATE) - ATE_true)^2 + sd(ATE)^2)) ,eta]
