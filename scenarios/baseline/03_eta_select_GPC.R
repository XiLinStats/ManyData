...
func <- function(eta) {
  ...
  theta_sim_raw <- run_MH_MCMC(MLE_exp, 5000, data_obs_b, data_exp_b,
                                          msks_obs,theta_obs,
                                          msks_exp,theta_exp,as.vector(c(0,0,0,0)), 0.2*diag(4),eta)

  theta_sim_rtnd <- theta_sim_raw[-seq(1:50),]
  #
  beta_1 <- quantile(theta_sim_rtnd[,3],probs = c(0.025,0.975))
  beta_3 <- quantile(theta_sim_rtnd[,4],probs = c(0.025,0.975))
  ATE_exp <- quantile(theta_sim_rtnd[,3] + theta_sim_rtnd[,4] * data_exp_mean_C1,probs = c(0.025,0.975))
  ATE_obs <- quantile(theta_sim_rtnd[,3] + theta_sim_rtnd[,4] * data_obs_mean_C1,probs = c(0.025,0.975))
  ATE_comb <- quantile(theta_sim_rtnd[,3] + theta_sim_rtnd[,4] * data_comb_mean_C1,probs = c(0.025,0.975))
