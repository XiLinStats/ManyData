...
run_sims <- function(include.oberst = TRUE, include.rosenman = TRUE){
  ...
  for (i in 1:length(bias_list)) {
    ...
    tryCatch({
      ...
      fit_exp <- fitCausal(dat = data_exp_b, formulas = forms2$exp, family = family[-2])

      fit_obs <- fitCausal(dat = data_obs_b, formulas = forms2$obs, family = family[-2] )
      ...
      if (include.oberst == TRUE) {
        ...
        ATE_vec <- as.vector(c(0,0,0,0,1,mean(data_exp_b$C1),0,0,0,0))
        theta_e <- t(ATE_vec) %*% fit_exp$par[1:np]
        theta_o <- t(ATE_vec) %*% fit_obs$par[1:np]
        var_e <-  t(ATE_vec) %*% fit_exp$sandwich[1:np,1:np] %*% ATE_vec
        var_o <-  t(ATE_vec) %*% fit_obs$sandwich[1:np,1:np] %*% ATE_vec
      ...
      ATE_true <- t(ATE_vec) %*% theta$exp[1:np]
      ...
      if (is.na(opt_samples)) {
        ...
        create_scoring_vec <- function(theta){
          res <- as.vector(c(theta[5],theta[6]))
          return(res)
        }

        true_vec <- create_scoring_vec(theta$exp)
        eta_vec <- create_scoring_vec(colMeans(opt_samples))
        e_vec <- create_scoring_vec(fit_exp$par)

        # model matrix for CATE
        mm2 <- as.matrix(cbind(rep(1, n_e),data_exp_b[,c("C1")]))

        # score data_exp_b for causal effects on each observation
        data_exp_b[, eff_eta := mm2 %*% eta_vec]
        data_exp_b[, eff_true := mm2 %*% true_vec]
        data_exp_b[, eff_e := mm2 %*% e_vec]

        # Implement method in Kallus et al. (2018)
        mm3 <- as.matrix(cbind(rep(1, n_e),data_exp_b[,c("C1","Z")]))
        ...
        eta_model <- glm(resid ~  C1  + Z , family = gaussian, data = data_exp_b)
...
saveRDS(results1,"data/sim_v6.rds")
...
dat <- results1
summ <- dat[,.(.N,MSE_oberst = mean((ATE_oberst - ATE_true)^2),
               MSE_0 = mean((theta_e - ATE_true)^2),
               MSE_eta = mean((ATE_eta - ATE_true)^2),
               MSE_gsar1 = mean((ATE_gsar1 - ATE_true)^2),
               MSE_gsar2 = mean((ATE_gsar2 - ATE_true)^2),
               MSE_kappa1 = mean((ATE_kappa1 - ATE_true)^2),
               MSE_kappa2 = mean((ATE_kappa2 - ATE_true)^2),
               MSE_kallus = mean((ATE_kallus - ATE_true)^2),
               mean_eta = mean(opt_eta),
               mean_lambda = mean(oberst_lambda)),bias]

summ[,':='(MSE_eta_ratio = MSE_eta/MSE_0,
           MSE_oberst_ratio = MSE_oberst/MSE_0,
           MSE_gsar1_ratio = MSE_gsar1/MSE_0,
           MSE_gsar2_ratio = MSE_gsar2/MSE_0,
           MSE_kapp1_ratio = MSE_kappa1/MSE_0,
           MSE_kapp2_ratio = MSE_kappa2/MSE_0,
           MSE_kallus_ratio = MSE_kallus/MSE_0)]
...
p1 <- ggplot(summ, aes(x = bias)) +
  ...
  # geom_line(aes(y = MSE_gsar2_ratio,colour = "gsar2"), size = 1.3) +
  geom_line(aes(y = MSE_kapp1_ratio,colour = "Rosenman et al. 2021"),  size = 1.3) +
  # geom_line(aes(y = MSE_kapp2_ratio,colour = "kappa2"),  size = 1.3) +
  geom_line(aes(y = MSE_kallus_ratio,colour = "Kallus et al. 2018"),  size = 1.3) +
  scale_colour_manual("",
                      breaks = c("Ours", "Oberst et al. 2022","Green et al. 2005", "gsar2","Rosenman et al. 2021","kappa2","Kallus et al. 2018","Parametric method\nbenchmark"),
                      values = c("#800000", "#002058", "#ed008c","#a79d96","#33b1e5","#0f7361","#f5ac3d","black")) +
  xlab("bias") +
...
ggsave(file.path(data_folder,"ATE_MSE.pdf"),
       plot = p1,
       device = cairo_pdf,
       dpi = 1280)

ggsave(file.path(data_folder,"CATE_MSE.pdf"),
       plot = p2,
       device = cairo_pdf,
       dpi = 1280)
